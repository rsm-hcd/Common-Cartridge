...
using CommonCartridge.Core.Models.v1_0;
using System;
using System.IO;
...
{
    public class V1_0Parser : IV1_0Parser
    {
        public BasicLTIParserResult FromLTIFile(string path)
        {
            ...
            try
            {
                ...
                result = FromLTIXml(File.ReadAllText(path));
        ...
        public BasicLTIParserResult FromLTIXml(string content)
        {
            var result = new BasicLTIParserResult();
            ...
            try
            {
                ...
                var serializer = new XmlSerializer(typeof(CartridgeBasicLTILinkType));
                using (var stream = new MemoryStream(Encoding.UTF8.GetBytes(content)))
                {
                    result.BasicLTI = serializer.Deserialize(stream) as CartridgeBasicLTILinkType;
                }
            }
            catch (Exception ex)
            {
